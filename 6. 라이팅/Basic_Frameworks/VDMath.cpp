...
VDMATRIX VDMATRIX::operator*(const VDMATRIX& other) const
{
	...
	{
		std::cout << "행 우선 행렬과 곱셈을 시도함. 최적화를 위해 Transposition()을 사용할 것" << std::endl;
		for (int i = 0; i < 4; ++i)
		{
			for (int j = 0; j < 4; ++j)
			{
...
void VDMATRIX::VDMatrixLookAtLH(const VDVECTOR& eye, const VDVECTOR& target, const VDVECTOR& up) // 사실 뭐가 어떻게 되는지 잘 모르겠음
{
	...
	m_matrix[0][3] = -xAxis * eye;
	m_matrix[1][3] = -yAxis * eye;
	m_matrix[2][3] = -zAxis * eye;
	m_matrix[3][3] = 1.0f;
}

void VDMATRIX::VDMatrixPerspectiveFovLH(float fov, float aspect, float nearZ, float farZ) // 예도 마찬가지
{
	m_isRowMajor = false;

	float yScale = 1.0f / tanf(fov * 0.5f);
	float xScale = yScale / aspect; // NDC 처리용 종횡비
	...
	m_matrix[3][2] = 1.0f;
	m_matrix[2][3] = -nearZ * farZ / (farZ - nearZ);
}

void VDMATRIX::PrintMatrix() const
{
	if (m_isRowMajor)
	{
		for (int i = 0; i < 4; ++i)
...
VDVECTOR VDVECTOR::Rotate(const VDMATRIX& matrix) const
{
	VDVECTOR result;
	if (matrix.m_isRowMajor)
	{
		std::cout << "행 우선 행렬과 곱셈을 시도함. 최적화를 위해 Transposition()을 사용할 것" << std::endl;
		result.x = this->x * matrix.m_matrix[0][0] + this->y * matrix.m_matrix[1][0] + this->z * matrix.m_matrix[2][0];
		result.y = this->x * matrix.m_matrix[0][1] + this->y * matrix.m_matrix[1][1] + this->z * matrix.m_matrix[2][1];
		result.z = this->x * matrix.m_matrix[0][2] + this->y * matrix.m_matrix[1][2] + this->z * matrix.m_matrix[2][2];
	}
	else
	{
		result.x = this->x * matrix.m_matrix[0][0] + this->y * matrix.m_matrix[0][1] + this->z * matrix.m_matrix[0][2];
		result.y = this->x * matrix.m_matrix[1][0] + this->y * matrix.m_matrix[1][1] + this->z * matrix.m_matrix[1][2];
		result.z = this->x * matrix.m_matrix[2][0] + this->y * matrix.m_matrix[2][1] + this->z * matrix.m_matrix[2][2];
	}
	return result;
}
