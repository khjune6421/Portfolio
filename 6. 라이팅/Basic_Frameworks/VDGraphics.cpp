...
int VooDooGraphics::_RenderTargetCreate()
{
	...
	m_depthBuffer.depthWidth = m_outputMerge.presentParameters.BackBufferWidth; // 깊이 버퍼 너비 설정
	m_depthBuffer.depthHeight = m_outputMerge.presentParameters.BackBufferHeight; // 깊이 버퍼 높이 설정
	m_depthBuffer.depthData.resize(m_depthBuffer.depthWidth * m_depthBuffer.depthHeight, 1.0f); // 깊이 버퍼 데이터 초기화
	
	SelectObject(m_outputMerge.hDC, m_outputMerge.hBackBuffer);
	ReleaseDC(m_outputMerge.hWnd, hDC);

	m_light.Direction = VDVECTOR(-1.0f, -2.0f, 3.0f); // 조명 방향 설정
	m_light.Color = VDCOLOR(1.0f, 1.0f, 1.0f, 1.0f); // 조명 색상 설정
...
int VooDooGraphics::_Clear(COLORREF col)
{
	...
	std::fill(m_depthBuffer.depthData.begin(), m_depthBuffer.depthData.end(), 1.0f); // 깊이 버퍼 초기화
...
int VooDooGraphics::_VertexPipeline(_In_ VDVECTOR iPos[3], _Out_ VDVECTOR oPos[3], _Inout_ VDCOLOR oColor[3])
{
	...
	VDVECTOR normal = ((iPos[1] - iPos[0]) ^ (iPos[2] - iPos[0])).Normalize();
	...
	for (UINT i = 0; i < m_inputSystem.VertexAmount; ++i)
	{
		...
		vsOutputs[i] = _TnL_VP(pos, color, normal); // 변환 및 조명 처리
	}
...
{
	int result = 0;
	result = _TnL_GP(iPos, oPos, oColor); // 기하학 파이프라인 처리
...
{
	VDVECTOR edge1 = v1 - v0; // v0에서 v1까지의 벡터
	VDVECTOR edge2 = v2 - v0; // v0에서 v2까지의 벡터
	VDVECTOR normal = edge1 ^ edge2; // 외적을 이용한 법선 벡터 계산

	VDVECTOR viewDir = VDVECTOR(0, 0, 1); // 뷰 방향 벡터 (카메라가 바라보는 방향)

	return (normal * viewDir) >= 0; // 법선 벡터와 뷰 방향 벡터의 내적이 0 이상이면 컬링하지 않음
}
...
VS_OUTPUT VooDooGraphics::_TnL_VP(VDVECTOR iPos, VDCOLOR iColor, VDVECTOR normal)
{
	VS_OUTPUT output;
	...
	VDVECTOR n = normal.Rotate(m_transform.transform[VDTS_WORLD]).Normalize();
	VDVECTOR lightDir = m_light.Direction.Normalize();

	float NdotL = n * (-lightDir);
	if (NdotL < 0.0f) { NdotL = 0.0f; } // 법선 벡터와 조명 방향 벡터의 내적이 음수이면 0으로 설정

	VDCOLOR diffuse = m_light.Color * NdotL;

	output.Color = diffuse * iColor; // 최종 색상 계산
...
int VooDooGraphics::_TnL_GP(_In_ VDVECTOR iPos4[3], _Out_ VDVECTOR oPos[3], _Inout_ VDCOLOR oColor[3])
{
	...
	if (_FaceCulling3(iPos4[0], iPos4[1], iPos4[2])) { return -2; } // 컬링 처리
...
int VooDooGraphics::_Face(VDVECTOR pos[3], VDCOLOR color[3])
{
	...
	for (int y = minY; y <= maxY; ++y)
	{
		for (int x = minX; x <= maxX; ++x)
		{
			...
			{
				float z = pos[0].z * alpha + pos[1].z * beta + pos[2].z * gamma;
				int idx = y * m_depthBuffer.depthWidth + x;

				if (idx < 0 || idx >= m_depthBuffer.depthData.size()) continue;
				if (z < m_depthBuffer.depthData[idx])
				{
					m_depthBuffer.depthData[idx] = z;
					VDCOLOR colorPixel = color[0] * alpha + color[1] * beta + color[2] * gamma;
					SetPixel(m_outputMerge.hDC, x, y, colorPixel);
				}
			}
		}
	}
