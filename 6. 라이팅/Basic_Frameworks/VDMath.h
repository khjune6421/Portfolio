...
public:
	...
	VDVECTOR Rotate(const VDMATRIX& matrix) const; // 벡터 회전
...
public:
	...
	void VDMatrixPerspectiveFovLH(float fov, float aspect, float nearZ, float farZ); // 원근 투영 행렬 생성

	// 기타 디버깅 함수
	void PrintMatrix() const; // 행렬 출력 함수
};
