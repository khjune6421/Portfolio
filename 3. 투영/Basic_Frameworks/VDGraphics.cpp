...
int VooDooGraphics::_Clear(COLORREF col)
{
	...
	RECT rect = { 0, 0, static_cast<long>(m_outputMerge.presentParameters.BackBufferWidth), static_cast<long>(m_outputMerge.presentParameters.BackBufferHeight) }; // 전체 화면 영역
...
{
	float w = static_cast<float>(m_outputMerge.presentParameters.BackBufferWidth);
	float h = static_cast<float>(m_outputMerge.presentParameters.BackBufferHeight);

	for (UINT i = 0; i < m_inputSystem.VertexAmount; ++i)
	{
		oPos[i].x = iPos[i].x * (w * 0.5f) + (w * 0.5f); // x 좌표 변환
		oPos[i].y = -iPos[i].y * (h * 0.5f) + (h * 0.5f); // y 좌표 변환
	}
...
int VooDooGraphics::_TnL_GP(_In_ VDVECTOR iPos4[3], _Out_ VDVECTOR oPos[3], _Inout_ VDCOLOR oColor[3])
{
	...
	if (m_inputSystem.VertexAmount >= 3)
	{
		if (_FaceCulling3(iPos4[0], iPos4[1], iPos4[2])) // 3D 컬링
		{
			return -1; // 컬링된 경우 에러
		}
	}
...
void VooDooGraphics::_DrawGrid()
{
	...
	VDCOLOR gridColorX(0, 1, 0, 1); // X 그리드 색상
	VDCOLOR gridColorZ(1, 0, 0, 1); // Z 그리드 색상

	VDVIEWPORT viewport = { 0, 0, static_cast<float>(m_outputMerge.presentParameters.BackBufferWidth), static_cast<float>(m_outputMerge.presentParameters.BackBufferHeight), 0.0f, 1.0f }; // 뷰포트 설정
