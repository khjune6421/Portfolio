...
VDDISPLAYMODE g_displayMode = { 800, 600 };
...
void DrawText(int x, int y, COLORREF col, const WCHAR* msg, ...)
{
	...
	SetTextColor(GetDC(g_hWnd), col);
	DrawText(GetDC(g_hWnd), buffer, static_cast<int>(wcslen(buffer)), &rect, DT_WORDBREAK);
	SetTextColor(GetDC(g_hWnd), g_fontColor);
}
...
void ShowFPS(int x, int y)
{
	...
	if (timeElapsed >= 1000)
	{
		framesPerSecond = static_cast<float>(frame) / (timeElapsed / 1000.0f);
		frame = 0;
		lastTime = currentTime;
	}
	DrawText(x, y, g_fontColor, L"FPS: %.2f", framesPerSecond);
}
